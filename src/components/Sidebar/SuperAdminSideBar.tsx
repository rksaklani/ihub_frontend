...
//           <h1 className='text-white text-2xl font-bold' >iHub Inventory</h1>
//         </NavLink>
  ...
  return (
    <aside
      ...
      className={`absolute left-0 top-0 z-9999 flex h-screen flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      } ${sidebarExpanded ? 'w-72.5' : 'w-20'}`} // Adjust width based on sidebarExpanded
    >
      {/* SIDEBAR HEADER */}
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        <NavLink to="/">
          {sidebarExpanded ? (
            ...
            // <h1 className="text-white text-2xl font-bold">iHub</h1> // Shortened logo when collapsed
             <img src={Logo} alt="Logo" /> 
        ...
        <nav className="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
               ...
               <SidebarLinkGroup
                activeCondition={
                  pathname === '/' || pathname.includes('/superadmin/dashboard')
                }
              >
                {(handleClick, open) => {
                  return (
